...
image: "images/gateway-api-end-to-end-on-k0s/featured-image.png"
---
...
Ingress served as the de facto routing standard for years and covered straightforward host and path routing well. The friction surfaced with everything beyond that: controllers like ingress-nginx extended behavior through annotations and configuration snippets, which started as useful flexibility but accumulated as security exposure. When the Kubernetes community [retired ingress-nginx in late 2025](https://kubernetes.io/blog/2025/11/11/ingress-nginx-retirement/), the announcement put it plainly: "Yesterday's flexibility has become today's insurmountable technical debt." Gateway API is the designated modern replacement, with advanced routing as first-class fields rather than annotations, and a role-oriented model that separates cluster-operator and application-developer concerns from the start.
...
`--skip-crds` is mandatory once Gateway API v1.5 is installed. The reason is a `ValidatingAdmissionPolicy` that Gateway API v1.5 introduced, called `safe-upgrades.gateway.networking.k8s.io` (confirmed in the [v1.5 release notes](https://github.com/kubernetes-sigs/gateway-api/releases)), which:
...
- [Retiring ingress-nginx](https://kubernetes.io/blog/2025/11/11/ingress-nginx-retirement/): the Kubernetes blog post announcing the retirement and recommending Gateway API as the migration path.
